...
p=ggplot(dat, aes(x=cd19, y=mala, color=subject))+
  geom_jitter(position = position_jitter(width = 1, height = 0.5))+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  ggtitle("MALAT1 VS CD19")+
  xlab("CD19")+
  ylab("MALAT1")
ggMarginal(p, dat, cd19, mala, type="density", margins="both")
...
for(i in 1:2){
  intercept[[i]]=rep(coefficients(lmod.LMwFE[[i]])[1], times=15)
  subject.intercept[[i]]=coefficients(lmod.LMwFE[[i]])[2:15]
  subject.intercept[[i]]=c(0,subject.intercept[[i]])
  intercept[[i]]=intercept[[i]]+subject.intercept[[i]]

  slope[i]=coefficients(lmod.LMwFE[[i]])[16]
}

plot(mala~cd19, data=dat, main="Model v Original Data")
for(j in 1:15)
{
  abline(a=intercept[[1]][j], b=slope[1], col=(20+j), lty=2, lwd=2)
}
...
####	ZIP Nested Model Comparisons	 ####

# Comparing Count Structure Components
lmod.ZIPbase=mixed_model(mala~1,
                       data=dat,
                       random = ~1|subject,
                       family = zi.poisson(),
                       zi_fixed = ~ 1)

lmod.ZIPcomp1=mixed_model(mala~1,
                          random = ~ 1+cd19|subject,
                          data=dat,
                          family = zi.poisson(),
                          zi_fixed = ~ 1)
anova(lmod.ZIPbase,lmod.ZIPcomp1)
# Implies that there is strong evidence to suggest FOR the inclusion of a random slope in the model


lmod.ZIPbase=mixed_model(mala~cd19,
                         data=dat,
                         random = ~1|subject,
                         family = zi.poisson(),
                         zi_fixed = ~ 1)

lmod.ZIPcomp1=mixed_model(mala~cd19,
                          random = ~ 1|subject,
                          data=dat,
                          family = zi.poisson(),
                          zi_fixed = ~ 1)
anova(lmod.ZIPbase,lmod.ZIPcomp1)
# Implies that there is strong evidence to suggest FOR the inclusion of a FIXED effect slope in the model
...
lmod.ZIPcomp1=mixed_model(mala~cd19,
                          ...
                          zi_fixed = ~ 1)
anova(lmod.ZIPbase,lmod.ZIPcomp1)
# Indicated that the inclusion of a Random slope and a fixed effect slope is highly evidenced


# Comparing Zero Structure Components
lmod.ZIPbase=mixed_model(mala~cd19,
                         data=dat,
                         random = ~1+cd19|subject,
                         family = zi.poisson(),
...
lmod.ZIPcomp1=mixed_model(mala~cd19,
                          ...
                          zi_random = ~1|subject)
anova(lmod.ZIPbase,lmod.ZIPcomp1)
# Implies that the inclusion of a random intercept in the Zero-part of the model is not advisable

#### Final Model:
lmod.ZIP=mixed_model(mala~cd19,
                         data=dat,
                         random = ~1+cd19|subject,
                         family = zi.poisson(),
                         zi_fixed = ~ 1)
